...
Sources: organizer brief "«Аким на 5 часов» – AI-симулятор управления городом" and the five-page dataset "Датасет районов". Expected numbers live in `test/acceptance/cases.json`, generated by `test/acceptance/oracle.py`, an independent Python implementation of the dataset rules (run `python3 test/acceptance/oracle.py`). The app itself does not use this oracle.
