...
2. Click **Try an example**, then **Simulate my city**. Show cost95, score56.54, zero critical indicators, and Nura’s schools/clinic/safety upgrades. Buildings and reactions illustrate district effects.
3. Click **Pin as Plan A**. Lock **School and kindergarten** using **Keep in advice**.
...
6. Click **Explain my plan** and open **View calculation evidence**. Point out live versus offline mode. AI selects relevant verified evidence; the engine calculates and validates every result.
...
- Launch from a fresh terminal using `./scripts/start.sh` or `node server/main.mjs`.
- `node --test`: all tests pass, zero skipped.
- Example and recommendation flow tested on the exact final commit.
- WebGL view and graceful fallback; readable laptop layout and basic small-screen controls.
- No secret in tracked files or JSON exports; API key only in ignored local environment.
- README matches implemented features; screenshot if helpful.
- Commit/push the final tested revision and verify the remote SHA.
- Nartay checks the event platform’s required fields and clicks its final submit action. Git push alone does not establish platform submission.
