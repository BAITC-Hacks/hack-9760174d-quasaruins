...
## R08: expanded spacing and architectural contrast

`MAP_SCALE=2` is applied once in the shared longitude/latitude projection. Districts, roads, water, parks, mapped landmarks, LRT and service placement all use that same projection. Original geographic coordinates and the scoring dataset are unchanged. Models, vertical dimensions and parcel clearances remain restrained world-space sizes; the detail module receives `modelScale:1`. The scene therefore has more room between geographic anchors without doubling every building.

The white background and unlit white ground match beyond the district boundaries. Terrain retains a subtle district tint and a separate soft shadow layer; no haze obscures the skyline. Darker asphalt lines, blue water, deep blue glazing, emerald trees and white/stone/terracotta/slate facades improve contrast. Buildings form small deterministic blocks near road samples, with window bands, mullions, plinths and roof details. Sparse taller forms stay more than three scene units from mapped landmark anchors; filler close to landmarks stays low. These are illustrative architectural forms, not a surveyed building inventory.

Thirty additional existing neighborhood service forms supplement the thirty primary service/project sites. Each modeled district gains one secondary school, clinic, safety post, utility building, civic centre and transit shelter. Their shared geometry is batched into instanced meshes; they persist through every plan state. Primary service sites alone receive project additions and upgrade labels. All primary, secondary and standalone intervention parcels are reserved before ordinary architecture is placed. Filler is capped at150 buildings per district, uses shared instanced geometry/colors for bodies and facade detail, and avoids water, parks, landmarks, LRT and service parcels. Decorative street trees are capped at260.

Camera focus accounts for expanded map spacing. The camera's viewport offset frames the subject in the clear area between HUD and cards. Fit city projects the actual district outlines to calculate a complete overview within that area; it does not rely on an overly conservative rectangular map bound. District and service labels avoid the larger card deck and top controls. District selection still raycasts against the original geographic district meshes.

### R08 checks

The temporary geometry harness executes the actual renderer logic with real vendored Three geometry and the served geography. It retains all R05 checks: permanent primary services through all nine quarters/views, all fourteen project types in every allowed district, valid footprints, exact lag/completion behavior, hover clearing and paused/reduced-motion module clock. Additional checks verify doubled geographic projection, thirty extra safe service parcels, bounded facade geometry on unreserved land, complete camera fit at1280×720/390×844/375×667 and actual raycaster selection of all five districts after the viewport offset. Integration-specific counts and the final module baseline are recorded in the worker handoff.

Browser on the checked R08 revision: actual WebGL and white seamless background, complete Fit city overview, district/service focus, pausedQ0 with30 primary labels/zero upgraded labels/no official score;4× replay ended56.54/+3.99/critical0. Before returned52.56 and zero upgraded labels. At390×844, visible map labels stayed above the view switch/card controls and the page had no horizontal overflow. The R08 checkpoint uses main0a12fc2 with the older R03 detail module; final R06/R09 transfer is A-owned and must rerun the combined geometry harness after integration. The current module produced345 facade buildings,16 taller forms and30 extra existing services. All76 existing tests pass.

## R12: drag placement and compact project badges

Pointer dragging a hand card onto a district proposes a selection through the shared official `validatePlan(...,{allowPartial:true})`. The displayed rejection is the complete official error text for count, budget, category limit, incompatibility and invalid district. No rejected proposal changes the plan. Sarayshyk explicitly explains that it is outside the five-district scenario; city-wide projects normalize the target to null and still affect only the five modeled districts. The two-project limit applies to category, never district. The published three-project Nura example remains valid.

A drag begins after an eight-pixel threshold. It captures the pointer, locks renderer interactions, clears the descriptive tooltip and uses a separate image ghost. Invalid/cancelled gestures animate back to their original card or badge; reduced motion removes that animation. Pointer release, pointercancel, lost capture, Escape, blur, document hiding, dialogs and plan re-renders all release the lock and pointer capture. Touch cards allow horizontal native deck scrolling, with vertical gestures reserved for placement. Click and keyboard placement remain available when dragging or WebGL is unavailable.

Selected projects leave the hand and appear as compact image buttons at projected district anchors. City-wide projects occupy a separate upper row above the city centre. Badges spread to avoid overlap; anchors outside the visible clear area clamp to its edge and use a dashed border. Clicking/focusing a badge reveals the short project detail and removal action. Dragging a badge onto the hand removes it; dragging it to another valid district retargets it. Pinned Plan A badges remain inspectable but cannot mutate the current plan. Card artwork is cached separately for hand and badges so comparisons cannot steal another live element's image.

Pointer descriptions follow the pointer, clamp inside the viewport and cannot intercept input. The click/keyboard detail shows title, cost and effect icons; Details reveals description, full indicator names, scope and delay. The translucent HUD is smaller. Reports initially show official score/delta, weakest district, critical count and actions; district indices, all ten indicators, the numeric score ledger, comparisons and briefing remain in expandable sections. Printing opens all report details before invoking the browser print flow; JSON payload and full numeric precision are unchanged.

### R12 validation

Browser checks on port3001 at1280×720 and390×844: native pointer placement put M1 in Nura; an incompatible M3 drop returned its image and displayed the exact official conflict with no change; returning the M1 badge to the hand restored zero cost/slots. With the full95-unit example, a sixth project returned the exact count and113/100 budget errors, retained all five badges and the original plan. Three Nura badges remain supported. Keyboard Enter on a card and the Almaty shortcut placed M1 for18 units. The pointer tooltip stayed inside the mobile viewport, computed `pointer-events:none`, and cleared during drag. Mobile page and report have no horizontal overflow; five projected badges remain separated.

The example at4× still completes at56.54/+3.99/zero critical indicators. The report opens collapsed and its district Details expands to expose the existing indices/indicator values. Browser warning/error logs were empty. All79 current engine/server/effects/acceptance tests pass. A temporary Node harness executes the actual candidate and drag-finish functions against the real validator: count/budget/category/incompatibility/invalid target/Sarayshyk, valid retargeting, immutable invalid plans, and cancellation/rejection/removal restoring the renderer lock all pass. Source syntax and whitespace checks pass.

Limitations: real touch hardware, OS-level reduced-motion propagation and native print output were not newly verified. The lifecycle harness exercises cancellation paths; automated browser gestures verify real pointer placement/removal. Renderer pan/zoom, effects and language changes belong to Claude/A and need combined integration verification. New English strings were sent to Claude for translations; this task does not edit the i18n module.

Final integration check on main `a1cd343` with Claude R13: card drag onto Nura and badge drag back to the hand left Nura's screen anchor exactly unchanged (550.81,184.32). A subsequent ordinary map drag moved it to(616.92,196.27), confirming the renderer was unlocked again. The compact language button cycled English→Kazakh→Russian→English while preserving the52.56 baseline and empty plan; the new return-to-hand message translated. Pinned Plan A and the current hand both retain loaded M7/M8 images simultaneously. City-wide badges use the upper row,53px above district badges in the tested mobile view. Renderer geometry/effects still receive A's separate final audit.
